...
cat('\n\n')
cat("*Fontes:* [**Plataforma Sucupira**](https://sucupira.capes.gov.br/sucupira/)")
cat('^[Termos nas nuvens correspondem àqueles encontrados nos títulos dos projetos de pesquisa, trabalho de conclusão de curso e produção intelectual, respectivamente.]')
```
...
for (i in 1:dim(artigos)[1]) {
  c.score.i <-
    as.numeric(citescore$x2021_cite_score[grep(gsub("-", "", artigos$ISSN[i]), citescore$print_issn)])
  if (is_empty(c.score.i)) {
    as.numeric(citescore$x2021_cite_score[grep(gsub("-", "", artigos$ISSN[i]), citescore$e_issn)])
  }
  c.score <-
...
table.2 <-
  t(table(
    ...
    as.logical(c(doi_unique$is_oa, my_dois_works$is_oa))
  ))
table.2 <- table.2[order(rownames(table.2)), ]
...
cat('\n')
cat('^[Dados sobre retratação são obtidos de PubMed API e CrossRef API.]')
```
