...
bsplus::bs_carousel(id = "temas-carouse",
            use_indicators = FALSE,
            use_controls = TRUE) %>%
...
text(
  x = (x1 + x2) / 2,
  y = (y1 + y2) / 2,
  labels = as.character(sum(na.omit(as.numeric(table.1[, 2])))),
  adj = 0.5
)
...
summ.table.visibilidade <-
  cbind(
    sum(na.omit(as.numeric(table.1[, 2]))),
    formatC(
...
# separating egressos (master and doctoral) and current candidates
discentes <-
