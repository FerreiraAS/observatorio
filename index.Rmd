...
---
title: "OBSERVATÓRIO `r ifelse(and(has.dados.cadastrais, has.sucupira), sigla.PPG, "")`"
---
...
```{r logo-ies}
if(file.exists("PPG/Images/logo-IES.png")) {
  cat('<img src=\"PPG/Images/logo-IES.png\" align=\"center\"/>')
}
```
...
if(and(has.dados.cadastrais, has.sucupira)){
cat(
  'O **Observatório ', toupper(as.character(sigla.PPG)),
  '** é uma iniciativa do **Programa de Pós-graduação em ', as.character(nome.PPG), '** - [', toTitleCase(as.character(nome.IES)), '](', as.character(site.PPG),')',
...
CITESCORE.base <- citescore$x2021_cite_score[!is.na(citescore$x2021_cite_score)]
...
top <- min(length(do::unique_no.NA(IES.nac)), 10)
data.nac <-
  na.omit(data.frame(name = names(sort(
...
top <- min(length(do::unique_no.NA(IES.inter)), 10)
data.inter <-
  na.omit(data.frame(name = names(sort(
...
top <- min(length(do::unique_no.NA(mundo)), 10)
data.pais <-
  na.omit(data.frame(name = names(sort(
