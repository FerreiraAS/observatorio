...
cat('\n\n')
cat("*Fontes:* [**Plataforma Sucupira**](https://sucupira.capes.gov.br/sucupira/)")
cat('^[Termos nas nuvens correspondem àqueles encontrados nos títulos dos projetos de pesquisa, traeblho de conclusã de curso e produç˜ão intelectual, respectivamente.]')
```
...
for (i in 1:dim(artigos)[1]) {
  c.score.i <-
    as.numeric(citescore[grep(gsub("-", "", artigos$ISSN[i]), citescore$issn), 6])
  c.score <-
...
table.4 <- data.frame(artigos$Publicado.em, SJR)
table.4.median <-
  aggregate(
    table.4,
