...
rownames(table.3) <- c("Open acess", "Paywall")
...
barplot(
  scale(
    as.matrix(table.3),
    center = FALSE,
    scale = colSums(as.matrix(table.3))
  ) * 100,
  ylab = "%",
  col = mycolors,
  ylim = c(0, 100)
...
summ.table.visibilidade <-
  cbind(
    sum(table.6[, 2]),
    formatC(
      min(table.6[, 2], na.rm = TRUE),
      format = "d",
      digits = 0,
      big.mark = "."
    ),
    formatC(
      median(table.6[, 2], na.rm = TRUE),
      format = "d",
      digits = 0,
      big.mark = "."
    ),
    formatC(
      max(table.6[, 2], na.rm = TRUE),
      format = "d",
      digits = 0,
      big.mark = "."
    )
