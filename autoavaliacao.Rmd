...
síntese.ano <- síntese.ano %>%
  dplyr::select(c(
    ...
    "Nível do Curso",
    "Nome da Disciplina",
    "Indicadora de disciplina obrigatória",
...
table.1.1.2 <-
  gtsummary::tbl_summary(
    ...
    by = "Ano",
    type = list(
      "Nível do Curso" ~ "continuous",
      "Nome da Disciplina" ~ "continuous",
      "Indicadora de disciplina obrigatória" ~ "continuous",
      "Quantidade de créditos" ~ "continuous",
      "Carga Horária" ~ "continuous"),
    statistic = list(
      all_continuous() ~ c("{max}")),
    missing = "no") %>%
  gtsummary::modify_header(
