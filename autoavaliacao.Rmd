...
##### **Qualitativo** {#ac-cp .tabset}

###### **Áreas de Concentração** {#descricao-ac}
...
###### **Linhas de Pesquisa** {#descricao-lp}
...
###### **Projetos de Pesquisa** {#descricao-pp}

```{r 1-1-1-data, eval = has.sucupira.files}
# projetos - membros
...
# remove duplicates
sucupira.df <- sucupira.df[!duplicated(sucupira.df[, -1]), ]

# select data DOCENTES
table.doc <-
  table(sucupira.df$`Nome do Membro do Projeto`,
        sucupira.df$`Linha de Pesquisa`)
# add row totals
...
# select data POS-DOC
table.posdoc <- NULL
if (any(sucupira.df$`Categoria do Membro do Projeto` == "Pós-Doc")) {
  table.posdoc <-
    table(sucupira.df$`Nome do Membro do Projeto`[sucupira.df$`Categoria do Membro do Projeto` == "Pós-Doc"],
          sucupira.df$`Linha de Pesquisa`[sucupira.df$`Categoria do Membro do Projeto` == "Pós-Doc"])
  # add row totals
...
##### **Quantitativo** {#coerencia-quantitativo}

```{r 1-1-1-ac, eval = has.sucupira.files}
table.ac <-
  data.frame(matrix(
    NA,
...
```{r 1-1-1-lp, eval = has.sucupira.files}
table.lp <-
  data.frame(matrix(
    NA,
...
```{r 1-1-1-pp, eval = has.sucupira.files}
table.pp <-
  data.frame(matrix(
    NA,
...
table.pp.gt <-
  ...
  remove_row_type(
    variables = c('Nome do Projeto de Pesquisa'),
    type = "level",
...
```{r 1-1-1-doc, eval = has.sucupira.files}
table.doc <-
  data.frame(matrix(
    NA,
    nrow = length(unique(sucupira.df$`Nome do Projeto de Pesquisa`)),
...
table.doc <- cbind(unique(sucupira.df$`Nome do Projeto de Pesquisa`), table.doc)
colnames(table.doc) <-
  c("Nome do Projeto de Pesquisa", as.character(quadrienal.vigente))
...
table.doc.gt <-
  ...
  remove_row_type(
    variables = c('Nome do Membro do Projeto'),
    type = "level",
    level_value = ("Membros")
  ) %>%
  modify_column_hide(columns = paste0("stat_", as.character(length(quadrienal.vigente) + 1)))
...
```{r 1-1-1-ac-lp-pp-doc, eval = has.sucupira.files}
...
```{r doc-detalhes, eval = has.sucupira.files}
options(knitr.kable.NA = '')
print(
