...
if (dim(egressos)[1] != 0) {
  ids <- order(egressos$'Data Situação', egressos$'Nome Discente', decreasing = c(TRUE, FALSE))
  table <- egressos[ids,]
  source("Scripts/table-with-buttons.R", local = knitr::knit_global())
...
if (dim(bolsistas)[1] != 0) {
  ids <- order(bolsistas$'Matrícula', bolsistas$'Nome do Autor', decreasing = c(TRUE, FALSE))
  table <- bolsistas[ids,]
  source("Scripts/table-with-buttons.R", local = knitr::knit_global())
