...
### **Lattes** {#lattes .tabset}

```{r docentes-atual-lattes, eval = has.sucupira.files}
# projetos - membros
sheet <- "Docentes"
  ...
  if (!is.na(docentes$`Número do currículo Lattes`[i])) {
    cat(
      paste0(
        ...
        '.pdf" style=\"width:100%; height:600px; border:2px solid black;"></iframe>'
      )
    )
  } else {
...
if (dim(discentes)[1] != 0) {
  ids <-
    order(as.character(discentes$'Tempo em curso'),
          discentes$'Nome Discente',
          decreasing = c(FALSE, FALSE), method = "radix")
  table <- discentes[ids,]
  source("Scripts/table-with-buttons.R", local = knitr::knit_global())
...
if (dim(egressos)[1] != 0) {
  ids <-
    order(as.character(egressos$'Data Situação'),
...
if (dim(bolsistas)[1] != 0) {
  ids <-
    order(as.character(bolsistas$'Matrícula'),
...
### **Grupos externos** {#grupos-externos .tabset}
