...
colnames(sucupira) <-
  colnames(sucupira.list[[as.character(max(periodo))]])
...
docentes <-
  sucupira[
    !duplicated(
      sucupira[, 
          match("Nome Docente", colnames(sucupira)):match("Carga Horária PG (Semanal)", colnames(sucupira))]
      ), ]
docentes <- docentes[order(docentes$"Nome Docente"),]
...
table.2 <- matrix(NA, nrow = 0, ncol = 6)
for (i in 1:dim(docentes)[1]) {
  table <- cbind(
    i,
    paste(
