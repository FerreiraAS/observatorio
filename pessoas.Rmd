  ...
  if (!is.na(docentes$`Número do currículo Lattes`[i])) {
    cat(
      paste0(
        ...
        '.pdf" style=\"width:100%; height:600px; border:2px solid black;"></iframe> height:600px; border:2px solid black;"></iframe>'
      )
    )
  } else {
...
egressos.raw <-
  readxl::read_excel(
    "PPG/Discentes.xlsx",
    sheet = "Discentes",
    col_types = c("text", "text", "date", "text", "date", "text", "text")
  )
egressos.raw <- egressos.raw[egressos.raw$País == "Brasil",]
egressos.raw$'Ano Titulação' <-
  substr(as.Date(egressos.raw$'Data Situação', format = "%Y-%m-%d"), 1, 4)
...
```{r historico-egressos-dsc-2024, eval = has.sucupira.files}
# periodo
periodo <- 2021:min(as.numeric(format(Sys.Date(), "%Y")), 2024)
...
# filter students
egressos <- egressos[str_detect(egressos$`Nível Discente`, course.level), ]
...
if (dim(egressos)[1] != 0) {
  ids <- order(egressos$'Data Situação', decreasing = TRUE)
  table <- egressos[ids,]
  source("Scripts/table-with-buttons.R", local = knitr::knit_global())
...
```{r historico-egressos-msc-2024, eval = has.sucupira.files}
# periodo
periodo <- 2021:min(as.numeric(format(Sys.Date(), "%Y")), 2024)
...
```{r historico-egressos-dsc-2020, eval = has.sucupira.files}
# periodo
periodo <- 2017:min(as.numeric(format(Sys.Date(), "%Y")), 2020)
...
```{r historico-egressos-msc-2020, eval = has.sucupira.files}
# periodo
periodo <- 2017:min(as.numeric(format(Sys.Date(), "%Y")), 2020)
...
```{r historico-egressos-dsc-2016, eval = has.sucupira.files}
# periodo
periodo <- 2013:min(as.numeric(format(Sys.Date(), "%Y")), 2016)
...
```{r historico-egressos-msc-2016, eval = has.sucupira.files}
# periodo
periodo <- 2013:min(as.numeric(format(Sys.Date(), "%Y")), 2016)
...
```{r historico-egressos-dsc-2012, eval = has.sucupira.files}
# periodo
periodo <- 2010:min(as.numeric(format(Sys.Date(), "%Y")), 2012)
...
```{r historico-egressos-msc-2012, eval = has.sucupira.files}
# periodo
periodo <- 2010:min(as.numeric(format(Sys.Date(), "%Y")), 2012)
...
### **Grupos de pesquisa externos** {#grupos-externos .tabset}
