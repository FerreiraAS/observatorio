...
# MODELOS
# Modelo bruto
model_bruto <- lm(razao ~ total_docentes, data = dados_modelo)

# Modelo ajustado por ano
model_ajustado <- lm(razao ~ total_docentes + ano, data = dados_modelo)
...
# Add LaTeX commands for new page and float barrier
if(knitr::is_latex_output() && !sjmisc::is_empty(tb_merged)) {
  cat('\\newpage')
}
```
