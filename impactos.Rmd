...
```{r pandemia-filtro-completos, eval = has.artigos.completos}
# dplyr::selecting those with COVID in the title
try(doi_unique_artigos_impacto <-
      doi_unique_artigos[grep(paste(tolower(pandemia), collapse = "|"),
                              tolower(doi_unique_artigos$title)), ], silent = TRUE)
try(my_doi_works_artigos_impacto <-
      my_doi_works_artigos[grep(paste(tolower(pandemia), collapse = "|"),
                                 tolower(my_doi_works_artigos$title)), ],
    silent = TRUE)
...
```{r pandemia-filtro-resumos, eval = has.artigos.resumos}
# dplyr::selecting those with COVID in the title
try(doi_unique_resumos_impacto <-
      doi_unique_resumos[grep(paste(tolower(pandemia), collapse = "|"),
                              tolower(doi_unique_resumos$title)),], silent = TRUE)
try(my_doi_works_resumos_impacto <-
      my_doi_works_resumos[grep(paste(tolower(pandemia), collapse = "|"),
                                 tolower(my_doi_works_resumos$title)),],
    silent = TRUE)
...
```{r pandemia-filtro-preprints, eval = has.preprints}
# dplyr::selecting those with COVID in the title
try(doi_unique_preprints_impacto <-
      doi_unique_preprints[grep(paste(tolower(pandemia), collapse = "|"),
                                tolower(doi_unique_preprints$title)),],
    silent = TRUE)
try(my_doi_works_preprints_impacto <-
      my_doi_works_preprints[grep(paste(tolower(pandemia), collapse = "|"),
                                   tolower(my_doi_works_preprints$title)),],
    silent = TRUE)
...
```{r pandemia-filtro-livros, eval = has.livros}
# dplyr::selecting those with COVID in the title
try(doi_unique_livros_impacto <-
      doi_unique_livros[grep(paste(tolower(pandemia), collapse = "|"),
                             tolower(doi_unique_livros$title)), ], silent = TRUE)
try(my_doi_works_livros_impacto <-
      my_doi_works_livros[grep(paste(tolower(pandemia), collapse = "|"),
                                tolower(my_doi_works_livros$title)), ],
    silent = TRUE)
...
```{r pandemia-filtro-capitulos, eval = has.capitulos}
# dplyr::selecting those with COVID in the title
try(doi_unique_capitulos_impacto <-
      doi_unique_capitulos[grep(paste(tolower(pandemia), collapse = "|"),
                                tolower(doi_unique_capitulos$title)), ],
    silent = TRUE)
try(my_doi_works_capitulos_impacto <-
      my_doi_works_capitulos[grep(paste(tolower(pandemia), collapse = "|"),
                                   tolower(my_doi_works_capitulos$title)), ],
    silent = TRUE)
...
```{r pandemia-filtro-eventos, eval = has.eventos}
# dplyr::selecting those with COVID in the title
try(doi_unique_eventos_impacto <-
      doi_unique_eventos[grep(paste(tolower(pandemia), collapse = "|"),
                              tolower(doi_unique_eventos$title)), ], silent = TRUE)
try(my_doi_works_eventos_impacto <-
      my_doi_works_eventos[grep(paste(tolower(pandemia), collapse = "|"),
                                 tolower(my_doi_works_eventos$title)), ],
    silent = TRUE)
try(references_eventos_impacto <-
      references_eventos[grep(paste(tolower(pandemia), collapse = "|"), tolower(references_eventos)[, 2]), ], silent = TRUE
)
```
...
# bind sources
try(doi_unique <- doi_unique_artigos_impacto, silent = TRUE)
try(my_doi_works <- my_doi_works_artigos_impacto, silent = TRUE)
try(references <- references_artigos_impacto, silent = TRUE)

# available years of publication
try(years <-
  na.omit(sort(unique(
    c(doi_unique$published_on, my_doi_works$published_on)
  ), decreasing = TRUE)), silent = TRUE)

# Verifica se há DOIs
if (max(dim(doi_unique)[1], 0) == 0 &
    max(dim(my_doi_works)[1], 0) == 0) {
  cat('\n')
} else {
...
if (max(dim(doi_unique)[1], 0) == 0 &
  ...
  for (year in 1:length(years)) {
    ...
    table.with.badges(
      ...
      my_doi_works = tryCatch(
        my_doi_works[my_doi_works$published_on == years[year], ],
        error = function(e) {
          my_doi_works <- list()
        }
      ),
...
# bind sources
try(doi_unique <- doi_unique_resumos_impacto, silent = TRUE)
try(my_doi_works <- my_doi_works_resumos_impacto, silent = TRUE)
...
```{r pandemia-preprints, eval = has.preprints}
# bind sources
try(doi_unique <- doi_unique_preprints_impacto, silent = TRUE)
try(my_doi_works <- my_doi_works_preprints_impacto, silent = TRUE)
...
```{r pandemia-Livros, eval = has.livros}
# bind sources
try(doi_unique <- doi_unique_livros_impacto, silent = TRUE)
try(my_doi_works <- my_doi_works_livros_impacto, silent = TRUE)
...
```{r pandemia-capitulos, eval = has.capitulos}
# bind sources
try(doi_unique <- doi_unique_capitulos_impacto, silent = TRUE)
try(my_doi_works <- my_doi_works_capitulos_impacto, silent = TRUE)
...
# bind sources
try(doi_unique <- doi_unique_eventos_impacto, silent = TRUE)
try(my_doi_works <- my_doi_works_eventos_impacto, silent = TRUE)
try(references <- references_eventos_impacto, silent = TRUE)
