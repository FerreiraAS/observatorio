...
periodicos.raw <- readxl::read_excel("PPG/Periódicos institucionais.xlsx", sheet = "Periódicos", col_types = c("text"))
...
for(i in 1:dim(periodicos.raw)[1]){
  if(!is.na(periodicos.raw[i,3])){
    Abrir[i] <- paste0('<a href="', periodicos.raw[i,3],'" target="_blank"', '>', "↗️" ,"</a>", sep = "")
  }
}
...
for (i in 1:dim(cep.vigentes.raw)[1]) {
  cep.vigentes.raw[i, 3] <-
    paste0('<a href="', cep.vigentes.raw[i,3],'" target="_blank"', '>', "↗️" ,"</a>")
}
...
for (i in 1:dim(cep.revogados.raw)[1]) {
  cep.revogados.raw[i, 3] <-
    paste0('<a href="', cep.revogados.raw[i,3],'" target="_blank"', '>', "↗️" ,"</a>")
}
...
for (j in 1:length(tabs)) {
  ...
  if (dim(premios)[1] > 0) {
    ...
    for (i in 1:dim(premios)[1]) {
      ...
      premios[i, 3] <-
        paste0('<a href="', premios[i,3],'" target="_blank"', '>', "↗️" ,"</a>")
    }
