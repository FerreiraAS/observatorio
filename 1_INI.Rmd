...
```{r try-apresentacao-abas}
# initialize all sheets
abas <- try(excel_sheets("PPG/Apresentação1.xlsx"), silent = TRUE)
# remove specific abas
abas <- abas[-match("Sucupira", abas)]
has.apresentacao.abas <- length(abas) != 0 & !is.na(abas)
```
...
```{r try-descricao}
has.descricao <-
  !is.error(try(read.delim("PPG/Descrição.txt"),
                silent = TRUE)
  )
```
...
has.docentes.posdocs <-
  !is.error(try(read_excel("PPG/Docentes e Pós-Docs.xlsx", sheet = 1),
                silent = TRUE)
  )
```
...
```{r try-donwloads}
has.downloads <-
  !is.error(try(read_excel("PPG/Donwloads.xlsx", sheet = 1),
                silent = TRUE)
  )
```
...
```{r try-inventario}
has.inventario <-
  !is.error(try(read_excel("PPG/Inventário.xlsx", sheet = 1),
                silent = TRUE)
  )
```
...
has.artigos.completos <-
  !is.error(try(read_excel("PPG/Produção.xlsx", sheet = "PI.completos"), silent = TRUE)
  )
...
has.preprints <-
  !is.error(try(read_excel("PPG/Produção.xlsx", sheet = "PI.preprints"), silent = TRUE)
  )
