...
```{r ppg-data, results = "hide", eval = and(has.dados.cadastrais, has.sucupira.files)}
...
# data filter
doi_artigos_completos_sucupira <- data.frame(DOI = matrix(NA, nrow = length(titles), ncol = 1))
for(i in 1:length(titles)){
  doi_artigos_completos_sucupira$DOI[i] <- sucupira.raw$`Valor do Item de Detalhamento`[
    which(and(
...
doi_artigos_completos_sucupira <- data.frame(
  doi_artigos_completos_sucupira[!is.na(doi_artigos_completos_sucupira)]
  )
names(doi_artigos_completos_sucupira) <- "DOI"
has.artigos.completos.sucupira <- length(doi_artigos_completos_sucupira) != 0
...
# data filter
doi_artigos_resumos_sucupira <- data.frame(DOI = matrix(NA, nrow = length(titles), ncol = 1))
for(i in 1:length(titles)){
  doi_artigos_resumos_sucupira$DOI[i] <- sucupira.raw$`Valor do Item de Detalhamento`[
    which(and(
      sucupira.raw$`Nome da Produção` == titles[i],
      sucupira.raw$`Item de Detalhamento` == "Número do DOI"
    ) == TRUE)
...
doi_artigos_resumos_sucupira <- data.frame(
  doi_artigos_resumos_sucupira[!is.na(doi_artigos_resumos_sucupira)]
  )
names(doi_artigos_resumos_sucupira) <- "DOI"
has.artigos.resumos.sucupira <- length(doi_artigos_resumos_sucupira) != 0
```
...
```{r metrics-artigos, results = "hide", cache = TRUE, cache.path = "cache/artigos/", eval = any(has.artigos.completos.ppg, has.artigos.completos.sucupira)}
...
doi_with_altmetric_artigos <- get_metrics(df = doi_with_altmetric, citescore, scimago, qualis)
doi_without_altmetric_artigos <- get_metrics(df = doi_without_altmetric, citescore, scimago, qualis)
...
```{r metrics-resumos, results = "hide", cache = TRUE, cache.path = "cache/resumos/", eval = any(has.artigos.resumos.ppg, has.artigos.resumos.sucupira)}
# bind PPG and SUCUPIRA DOIs
dois.ppg <- data.frame()
if(has.artigos.resumos.ppg) {
  # read data
...
# store to use downstream
dois_resumos <- dois
doi_with_altmetric_resumos <- get_metrics(df = doi_with_altmetric, citescore, scimago, qualis)
doi_without_altmetric_resumos <- get_metrics(df = doi_without_altmetric, citescore, scimago, qualis)

# obtain formatted references
formatted_citations <-
  rcrossref::cr_cn(
    c(doi_with_altmetric$doi, doi_without_altmetric$doi),
    format = "text",
    raw = TRUE,
    style = "american-medical-association"
  )
...
# store to use downstream
dois_preprints <- dois
doi_with_altmetric_preprints <- get_metrics(df = doi_with_altmetric, citescore, scimago, qualis)
doi_without_altmetric_preprints <- get_metrics(df = doi_without_altmetric, citescore, scimago, qualis)
...
# store to use downstream
dois_livros <- dois
doi_with_altmetric_livros <- get_metrics(df = doi_with_altmetric, citescore, scimago, qualis)
doi_without_altmetric_livros <- get_metrics(df = doi_without_altmetric, citescore, scimago, qualis)
...
# store to use downstream
dois_capitulos <- dois
doi_with_altmetric_capitulos <- get_metrics(df = doi_with_altmetric, citescore, scimago, qualis)
doi_without_altmetric_capitulos <- get_metrics(df = doi_without_altmetric, citescore, scimago, qualis)
...
# store to use downstream
dois_eventos <- dois
doi_with_altmetric_eventos <- get_metrics(df = doi_with_altmetric, citescore, scimago, qualis)
doi_without_altmetric_eventos <- get_metrics(df = doi_without_altmetric, citescore, scimago, qualis)
...
```{r bind-all, results = "hide", cache = TRUE, cache.path = "cache/bind-all/", eval = any(has.producao.ppg, has.producao.sucupira)}
# bind sources
...
for (i in 1:dim(doi_with_altmetric_all)[1]) {
  if (!retractcheck::check_doi(doi = as.character(doi_with_altmetric_all$doi[i]))) {
    dois_unique_retracted <-
      rbind(dois_unique_retracted, doi_with_altmetric_all[i,])
  }
}
...
for (i in 1:dim(doi_without_altmetric_all)[1]) {
  if (!retractcheck::check_doi(doi = as.character(doi_without_altmetric_all$doi[i]))) {
    doi_without_altmetric_retracted <-
      rbind(doi_without_altmetric_retracted, doi_without_altmetric_all[i,])
  }
}
```
