...
```{r try-producao-sucupira, eval = has.sucupira}
# produções
...
doi_artigos_completos_sucupira <- 
  as.data.frame(produtos.completos$"Valor do Item de Detalhamento"[
...
doi_artigos_resumos_sucupira <- 
  as.data.frame(produtos.resumos$"Valor do Item de Detalhamento"[
...
dois <- as.data.frame(na.omit(bind_rows(dois, doi_artigos_completos_sucupira)))
dois <- as.data.frame(dois$DOI[!duplicated(dois$DOI)])
colnames(dois) <- "DOI"
...
# bind PPG and SUCUPIRA DOIs
dois <- as.data.frame(na.omit(bind_rows(dois, doi_artigos_resumos_sucupira)))
dois <- as.data.frame(dois$DOI[!duplicated(dois$DOI)])
colnames(dois) <- "DOI"
