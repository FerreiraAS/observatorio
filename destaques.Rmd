...
topics <- c()
destaques.list <- vector(mode = "list", length = length(tabs))
...
## **Produções de destaque (top-20)** {#produtos-destaque}

<!--script for generating HORIZONTAL LINE-->
<hr style="height:2px;border-width:0;color:black;background-color:black">

### **Atenção e visibilidade** {.tabset}
...
for (j in 1:length(abas)) {
  ...
  if (dim(premios.raw)[1] != 0) {
    ...
    cat(paste0('<iframe src=\"', link.tese, '\" style=\"width:100%; height:600px; border:2px solid black;"></iframe> height:600px; border:2px solid black;"></iframe>', "\n"))
    cat('\n\n<!-- -->\n\n')
    cat('\n</center>')
    cat('</div>')
    ...
    cat(paste0('<iframe src=\"', link.mencao, '\" style=\"width:100%; height:600px; border:2px solid black;"></iframe> height:600px; border:2px solid black;"></iframe>', "\n"))
    cat('\n</center>')
    cat('</div></div>')
  } else {
    cat("*Sem dados para exibir*")
...
# read separate sheets
file.path <- "PPG/Blog externo.xlsx"
tabs <- excel_sheets(file.path)
topics <- c()
news.list <- vector(mode = "list", length = length(tabs))
