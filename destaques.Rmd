...
for (j in 1:length(abas)) {
  ...
  if (dim(premios.raw)[1] != 0) {
    ...
    cat(paste0('<iframe src=\"', link.tese, '\" style="width:100%; height:500px; border:0;"></iframe>', "\n"))
    cat('\n\n<!-- -->\n\n')
    cat('\n</center>')
    cat('</div>')
    ...
    cat(paste0('<iframe src=\"', link.mencao, '\" style="width:100%; height:500px; border:0;"></iframe>', "\n"))
    cat('\n</center>')
    cat('</div></div>')
  } else {
    cat("*Sem dados para exibir*")
