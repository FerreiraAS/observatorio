...
```{r dados-cadastrais-programa, eval = has.dados.cadastrais}
...
# print tables
table <-
  unname(as.data.frame(dados.do.programa.raw, endereco.raw))
...
```{r projetos-de-pesquisa-2024-figura, eval = all(has.sucupira, has.avaliacao)}
...
if (dim(projetos.doc)[1] != 0) {
  pie(
    ...
    labels = NA,
    col = brewer.pal(n = length(levels), name = "Set3"),
    init.angle = 90,
    main = "Docentes"
  )
} else {
...
legend(
  ...
  inset = 0,
  legend = levels,
  fill = brewer.pal(n = length(levels), name = "Set3"),
  box.lwd = 0,
  cex = 0.7
)
...
```{r projetos-de-pesquisa-2024-tabela, eval = all(has.sucupira, has.avaliacao)}
periodo <- as.character(seq(min(periodos), max(periodos), by = 1))
projetos.all <- do.call(rbind.data.frame, sucupira.list[as.character(periodos)])
projetos <-
  projetos.all[projetos.all$`Membro Responsável` == "Sim" &
...
```{r projetos-de-pesquisa-2024-lista, eval = all(has.sucupira, has.avaliacao)}
periodo <- as.character(seq(min(periodos), max(periodos), by = 1))
projetos.all <- do.call(rbind.data.frame, sucupira.list[as.character(periodos)])
projetos <-
  projetos.all[projetos.all$`Membro Responsável` == "Sim" &
...
```{r projetos-de-pesquisa-2020-figura, eval = all(has.sucupira, has.avaliacao)}
periodo <- as.character(seq(min(periodos), max(periodos), by = 1))
projetos.all <- do.call(rbind.data.frame, sucupira.list[as.character(periodos)])
projetos <-
  projetos.all[projetos.all$`Membro Responsável` == "Sim" &
...
```{r projetos-de-pesquisa-2020-tabela, eval = all(has.sucupira, has.avaliacao)}
periodo <- as.character(seq(min(periodos), max(periodos), by = 1))
projetos.all <- do.call(rbind.data.frame, sucupira.list[as.character(periodos)])
projetos <-
  projetos.all[projetos.all$`Membro Responsável` == "Sim" &
...
```{r projetos-de-pesquisa-2020-lista, eval = all(has.sucupira, has.avaliacao)}
periodo <- as.character(seq(min(periodos), max(periodos), by = 1))
projetos.all <- do.call(rbind.data.frame, sucupira.list[as.character(periodos)])
projetos <-
  projetos.all[projetos.all$`Membro Responsável` == "Sim" &
...
```{r projetos-de-pesquisa-2016-figura, eval = all(has.sucupira, has.avaliacao)}
periodo <- as.character(seq(min(periodos), max(periodos), by = 1))
projetos.all <- do.call(rbind.data.frame, sucupira.list[as.character(periodos)])
projetos <-
  projetos.all[projetos.all$`Membro Responsável` == "Sim" &
...
```{r projetos-de-pesquisa-2016-tabela, eval = all(has.sucupira, has.avaliacao)}
periodo <- as.character(seq(min(periodos), max(periodos), by = 1))
projetos.all <- do.call(rbind.data.frame, sucupira.list[as.character(periodos)])
projetos <-
  projetos.all[projetos.all$`Membro Responsável` == "Sim" &
...
```{r projetos-de-pesquisa-2016-lista, eval = all(has.sucupira, has.avaliacao)}
periodo <- as.character(seq(min(periodos), max(periodos), by = 1))
projetos.all <- do.call(rbind.data.frame, sucupira.list[as.character(periodos)])
projetos <-
  projetos.all[projetos.all$`Membro Responsável` == "Sim" &
...
```{r projetos-de-pesquisa-2012-figura, eval = all(has.sucupira, has.avaliacao)}
periodo <- as.character(seq(min(periodos), max(periodos), by = 1))
projetos.all <- do.call(rbind.data.frame, sucupira.list[as.character(periodos)])
projetos <-
  projetos.all[projetos.all$`Membro Responsável` == "Sim" &
...
```{r projetos-de-pesquisa-2012-tabela, eval = all(has.sucupira, has.avaliacao)}
periodo <- as.character(seq(min(periodos), max(periodos), by = 1))
projetos.all <- do.call(rbind.data.frame, sucupira.list[as.character(periodos)])
projetos <-
  projetos.all[projetos.all$`Membro Responsável` == "Sim" &
...
```{r projetos-de-pesquisa-2012-lista, eval = all(has.sucupira, has.avaliacao)}
periodo <- as.character(seq(min(periodos), max(periodos), by = 1))
projetos.all <- do.call(rbind.data.frame, sucupira.list[as.character(periodos)])
projetos <-
  projetos.all[projetos.all$`Membro Responsável` == "Sim" &
...
# initialize all sheets
abas <- excel_sheets("PPG/Bolsas.xlsx")
...
for (i in 1:length(abas)) {
  ...
  for (i in 1:dim(bolsas.raw)[1]) {
    bolsas.raw[i, 3] <-
      paste("[↗️](", bolsas.raw[i, 3], '){target="_blank"}', sep = "")
  }
  
  print(
    kable(bolsas.raw, align = c("l", rep(
      "c", ncol(bolsas.raw) - 2
    ), "r")) %>% kable_styling(
      bootstrap_options = c("striped", "hover", "condensed", "responsive"),
      full_width = T,
...
# read separate sheets
creditos.raw <- readxl::read_excel("PPG/Dados Cadastrais.xlsx", sheet = "Créditos", col_types = c("text"))
...
print(
  kable(
    unique(table),
    row.names = FALSE,
    align = c("l", rep("c", ncol(table) - 2), "r")
  ) %>% kable_styling(
    bootstrap_options = c("striped", "hover", "condensed", "responsive"),
    full_width = T,
    position = "center"
...
```{r turmas-2024-2021, eval = has.sucupira}
# turmas
sheet <- "Turmas"
source("Scripts/read-xlsx-sucupira.R", local = knitr::knit_global())
...
```{r turmas-2024, eval = all(has.sucupira, has.avaliacao)}
# load custom function
source("Scripts/get-initials.R", local = knitr::knit_global())
...
```{r turmas-2020-2017, eval = has.sucupira}
# turmas
sheet <- "Turmas"
source("Scripts/read-xlsx-sucupira.R", local = knitr::knit_global())
...
```{r turmas-2020, eval = all(has.sucupira, has.avaliacao)}
# load custom function
source("Scripts/get-initials.R", local = knitr::knit_global())
...
```{r turmas-2016-2013, eval = has.sucupira}
# turmas
sheet <- "Turmas"
source("Scripts/read-xlsx-sucupira.R", local = knitr::knit_global())
...
```{r turmas-2016, eval = all(has.sucupira, has.avaliacao)}
# load custom function
source("Scripts/get-initials.R", local = knitr::knit_global())
...
```{r turmas-2012-2010, eval = has.sucupira}
# turmas
sheet <- "Turmas"
source("Scripts/read-xlsx-sucupira.R", local = knitr::knit_global())
...
```{r turmas-2012, eval = all(has.sucupira, has.avaliacao)}
# load custom function
source("Scripts/get-initials.R", local = knitr::knit_global())
...
# read separate sheets
turmas.raw <- readxl::read_excel("PPG/Calendários.xlsx", sheet = "Sede", col_types = c("text"))
...
## **Editais de processo seletivo** {.tabset #editais}
...
# read data
abas <- excel_sheets("PPG/Laboratórios.xlsx")
...
## **Cooperação interinstitucional** {.tabset #cooperacao}
...
# read separate sheets
file.path <- "PPG/Cooperações.xlsx"
tabs <- excel_sheets(file.path)
topics <- c()
cooperacao.list <- vector(mode = "list", length = length(tabs))
...
for (j in 1:length(tabs)) {
  ...
  # replace the link by a tag
  for (i in 1:dim(cooperacao)[1]) {
    cooperacao[i, 1] <-
      paste("**", cooperacao[i, 1], "**", sep = "")
    cooperacao[i, 3] <-
      paste("[↗️](", cooperacao[i, 3], '){target="_blank"}', sep = "")
  }
  
  # store data to print table
  cooperacao.list[[j]] <- cooperacao
}
...
for (j in 1:length(tabs)) {
  ...
  cat('\n\n<!-- -->\n\n')
  print(
    kable(cooperacao.list[[j]], align = c("l", rep(
      "c", ncol(premios) - 2
    ), "r")) %>% kable_styling(
