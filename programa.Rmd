...
most.recent <-
  as.character(ifelse(!is.null(sucupira.list[[format(Sys.Date(), "%Y")]]), sucupira.list[[format(Sys.Date(), "%Y")]], max(as.numeric(
    names(sucupira.list)
  ))))
...
```{r turmas-disciplinas-2024, eval = has.sucupira}
# load custom function
source("Scripts/get-initials.R", local = knitr::knit_global())
...
# read separate sheets
turmas.raw <- sucupira.list[[most.recent]]
