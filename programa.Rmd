...
for (i in 1:length(abas)) {
  ...
  historico <- cbind(historico.raw[, -3], Abrir)
  historico <- historico[order(historico$Publicação, decreasing = TRUE),]
  # print table
  print(
    kable(historico, align = "l", col.names = NULL) %>% kable_styling(
      bootstrap_options = c("striped", "hover", "condensed", "responsive"),
      full_width = T,
      position = "center"
    )
