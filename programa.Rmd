...
table.doc <-
  table(
    projetos.doc$`Área de Concentração`,
    projetos.doc$`Linha de Pesquisa`,
    projetos.doc$`Nome do Projeto de Pesquisa`,
    projetos.doc$`Nome do Membro do Projeto`
  )
# add row totals
table.doc <- cbind(table.doc, rowSums(table.doc))
colnames(table.doc) <- c(head(colnames(table.doc), -1), "N")
...
if (dim(projetos.posdoc)[1] != 0) {
  table.posdoc <-
    table(
      projetos.posdoc$`Área de Concentração`,
      projetos.posdoc$`Linha de Pesquisa`,
      projetos.posdoc$`Nome do Projeto de Pesquisa`,
      projetos.posdoc$`Nome do Membro do Projeto`
    )
  # add row totals
  table.posdoc <- cbind(table.posdoc, rowSums(table.posdoc))
  colnames(table.posdoc) <- c(head(colnames(table.posdoc), -1), "N")
